...
from networkx import minimum_node_cut, resistance_distance
import extmem
...
def Sort_Merge_Join(buf:extmem.Buf):
    ...
    # while r_idx < 16 * 4 and s_idx < 32 * 4:
    #     data0, data2 = int(r_data[r_idx % 4].split()[0]), int(s_data[s_idx % 4].split()[0])
    #     if data0 == data2:  # 先记录原位置，然后向右滑动
    #         res.append('%s %s' % (r_data[r_idx % 4], s_data[s_idx % 4]))
    #         if len(res) == floor(4 / 2):  # 结果缓冲区块已满
    #             buffer.writeBlockToDisk(res, '%srs%d.blk' % (sort_dir, count))
    #             res, count = [], count + 1
    #         idx_temp = s_idx + 1  # S变量临时向右滑动
    #         while idx_temp < 32 * 4:
    #             if not idx_temp % 4:
    #                 buffer.freeBlockInBuffer(1)
    #                 s_data = buffer.data[buffer.readBlockFromDisk('%ss%d.blk' % (sort_temp_dir, floor(idx_temp / 4)))]
    #             if data0 == int(s_data[idx_temp % 4].split()[0]):
    #                 res.append('%s %s' % (r_data[r_idx % 4], s_data[idx_temp % 4]))
    #                 idx_temp += 1  # 继续滑动
    #                 if len(res) == int(4 / 2):  # 结果缓冲区块已满
    #                     buffer.writeBlockToDisk(res, '%srs%d.blk' % (sort_dir, count))
    #                     res, count = [], count + 1
    #             else:
    #                 break
    #         if floor(idx_temp / 4) > floor(s_idx / 4):  # 如果关系S临时滑动到了新的一块
    #             buffer.freeBlockInBuffer(1)
    #             s_data = buffer.data[buffer.readBlockFromDisk('%ss%d.blk' % (sort_temp_dir, floor(s_idx / 4)))]
    #         idx_temp = r_idx + 1  # 关系R临时向右滑动
    #         while idx_temp < 16 * 4:
    #             if not idx_temp % 4:
    #                 buffer.freeBlockInBuffer(0)
    #                 r_data = buffer.data[buffer.readBlockFromDisk('%sr%d.blk' % (sort_temp_dir, floor(idx_temp / 4)))]
    #             if int(r_data[idx_temp % 4].split()[0]) == data2:
    #                 res.append('%s %s' % (r_data[idx_temp % 4], s_data[s_idx % 4]))
    #                 idx_temp += 1
    #                 if len(res) == int(4 / 2):
    #                     buffer.writeBlockToDisk(res, '%srs%d.blk' % (sort_dir, count))
    #                     res, count = [], count + 1
    #             else:
    #                 break
    #         if floor(idx_temp / 4) > floor(r_idx / 4):  # 如果关系R临时滑动到了新的一块
    #             buffer.freeBlockInBuffer(0)
    #             r_data = buffer.data[buffer.readBlockFromDisk('%sr%d.blk' % (sort_temp_dir, floor(r_idx / 4)))]
    #         r_idx, s_idx = r_idx + 1, s_idx + 1  # R和S向右滑动
    #         if not r_idx % 4 and r_idx < 16 * 4:
    #             buffer.freeBlockInBuffer(0)
    #             r_data = buffer.data[buffer.readBlockFromDisk('%sr%d.blk' % (sort_temp_dir, floor(r_idx / 4)))]
    #         if not s_idx % 4 and s_idx < 32 * 4:
    #             buffer.freeBlockInBuffer(1)
    #             s_data = buffer.data[buffer.readBlockFromDisk('%ss%d.blk' % (sort_temp_dir, floor(s_idx / 4)))]
    #     elif data0 > data2:
    #         s_idx += 1
    ...
    #             r_data = buffer.data[buffer.readBlockFromDisk('%sr%d.blk' % (sort_temp_dir, floor(r_idx / 4)))]
    # if res:
    #     buffer.writeBlockToDisk(res, '%srs%d.blk' % (sort_dir, count))  # 将结果磁盘上的剩余数据写入磁盘
    ...
    for i in range(int(num)):
        ...
        for j in range(8):
            ...
            index=buf.readBlockFromDisk(extmem.disk_dir+'r'+str(i*8+j)+'.blk')
            data+=buf.data[index]
        ...
        if num2==8 and flag2==0:
            ...
            for data in buf.data[index1]:
                ...
                if len(buf.data[index_res])==7:
                    buf.writeBlockToDisk(index_res,sort_sorted_dir+'r'+str(num_res)+'.blk')
                    num_res+=1
                    index_res=buf.getNewBlockBuffer()
            while True:
                if num1==8:
                ...
                index1=buf.readBlockFromDisk(sort_temp_dir+'r'+str(0)+str(num1)+'.blk')
                num1+=1
                for data in buf.data[index1]:
                    buf.data[index_res].append(data)
                    if len(buf.data[index_res])==7:
    ...
    for i in range(int(num)):
        data=[]
        for j in range(8):
            index=buf.readBlockFromDisk(extmem.disk_dir+'s'+str(i*8+j)+'.blk')
            data+=buf.data[index]
    ...
    while 1:
        ...
        if int(buf.data[R_index][0].split(' ')[0])==int(buf.data[S_index][0].split(' ')[0]):
            # print(2)
            buf.data[res_index].append(buf.data[R_index][0]+' '+buf.data[S_index][0].split(' ')[1])
            ...
            if len(buf.data[S_index])==1:
                if S_num+temp_num==32:
                    ...
                    temp_blk=buf.readBlockFromDisk(sort_sorted_dir+'s'+str(S_num+temp_num)+'.blk')
            else:
                flag=0
                for i in range(1,len(buf.data[S_index])):
                    if int(buf.data[R_index][0].split(' ')[0])==int(buf.data[S_index][i].split(' ')[0]):
                        buf.data[res_index].append(buf.data[R_index][0]+' '+buf.data[S_index][i].split(' ')[1])
                        if len(buf.data[res_index])==tumple_num:
                            # print(1)
                            res_file=sort_dir+'j'+str(res_num)+'.blk'
                            buf.writeBlockToDisk(res_index,res_file)
                ...
                while 1:
                    for i in range(len(buf.data[temp_blk])):
                        if int(buf.data[R_index][0].split(' ')[0])==int(buf.data[temp_blk][i].split(' ')[0]):
                            buf.data[res_index].append(buf.data[R_index][0]+' '+buf.data[temp_blk][i].split(' ')[1])
                            if len(buf.data[res_index])==tumple_num:
                                # print(1)
                                res_file=sort_dir+'j'+str(res_num)+'.blk'
                                buf.writeBlockToDisk(res_index,res_file)
